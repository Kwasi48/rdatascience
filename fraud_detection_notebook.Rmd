...
```{r}
confusion_matrix <- caret::confusionMatrix(test_set$isFraud, model.prediction)
confusion_matrix
```
...
```{r}
recal <- ModelMetrics::recall(test_set$isFraud, model.prediction)
recal
```
...
```{r}
preci <- ModelMetrics::precision( test_set$isFraud,model.prediction,)
preci
```
...
```{r}
f1_score <- ModelMetrics::f1Score(model.prediction, test_set$isFraud)
f1_score
```
...
```{r}
confusion_matrix <- caret::confusionMatrix(set_test$isFraud, pred_test)
confusion_matrix
```

```{r}
recal <- ModelMetrics::recall(test_set$isFraud, pred_test)
recal
```
